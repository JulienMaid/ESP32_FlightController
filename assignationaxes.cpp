...
void AssignationAxes::ReAssignation(const int16_t *i_i16_InputXYZ, int16_t *o_i16_OutputXYZ)
{
  ...
  for (auto l_u8_indexBoucle = 0; l_u8_indexBoucle < e_RepereOrthonormal_t::NbreAxes;
      l_u8_indexBoucle++)
  {
    ...
    for (auto l_u8_indexBoucle2 = 0; l_u8_indexBoucle2 < e_RepereOrthonormal_t::NbreAxes;
        l_u8_indexBoucle2++)
    {
      if (l_pi8_AxeRef[l_u8_indexBoucle2] != 0)
      {
        o_i16_OutputXYZ[l_u8_indexBoucle2] = l_pi8_AxeRef[l_u8_indexBoucle2]
            * i_i16_InputXYZ[l_u8_indexBoucle];
      }
    }
  }
